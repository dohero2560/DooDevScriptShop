class AdminPanel {
    ...
    async loadTopUps() {
        try {
            ...
            const container = document.getElementById('topupsContainer');
            container.innerHTML = `
                <h2>การเติมเงิน</h2>
                <div class="topups-list">
                    ${topups.map(topup => `
                        <div class="topup-item ${topup.status}">
                            <div class="topup-info">
                                <p>ผู้ใช้: ${topup.userId.username}#${topup.userId.discriminator}</p>
                                <p>จำนวนเงิน: ${topup.amount} บาท</p>
                                <p>วันที่: ${new Date(topup.createdAt).toLocaleString()}</p>
                                <p>สถานะ: ${topup.status}</p>
                                ${topup.slipData ? `
                                    <div class="slip-data">
                                        <p>ข้อมูลสลิป:</p>
                                        <p>ธนาคาร: ${topup.slipData.bankName || 'N/A'}</p>
                                        <p>จำนวนเงิน: ${topup.slipData.transferAmount || 'N/A'}</p>
                                        <p>วันที่โอน: ${topup.slipData.transferDate ? new Date(topup.slipData.transferDate).toLocaleString() : 'N/A'}</p>
                                    </div>
                                ` : ''}
                            </div>
                            <div class="slip-image">
                                <img src="/${topup.slipPath}" alt="Payment Slip">
                            </div>
                            ${topup.status === 'pending' ? `
                                <div class="topup-actions">
                                    <button onclick="adminPanel.processTopUp('${topup._id}', 'approved')" class="btn btn-success">
                                        อนุมัติ
                                    </button>
                                    <button onclick="adminPanel.processTopUp('${topup._id}', 'rejected')" class="btn btn-danger">
                                        ปฏิเสธ
                                    </button>
                                    <textarea placeholder="หมายเหตุ" id="comment-${topup._id}"></textarea>
                                </div>
                            ` : ''}
                        </div>
                    `).join('')}
            ...
            showNotification('ไม่สามารถโหลดข้อมูลการเติมเงินได้', 'error');
        }
    }

    async processTopUp(topupId, status) {
        try {
            const comment = document.getElementById(`comment-${topupId}`).value;
            ...
            if (response.ok) {
                showNotification('อัพเดทสถานะการเติมเงินสำเร็จ', 'success');
                this.loadTopUps();
            } else {
            ...
            showNotification('ไม่สามารถอัพเดทสถานะการเติมเงินได้', 'error');
        }
    }
}
